...
- 基調色は**白黒（モノクローム）**。primary は 船内=白 / 陸上=黒。押せる面と情報面は
  塗り・枠線・コントラスト差で区別し、色相で意味を作らない。
...
- 船内テーマの副次テキストは `text-foreground-400` を使う（`-500` は AA 4.5:1 未達）。
  検証値: 見出し 16.2:1 / 本文 6.8:1 / 主ボタン 17.6:1。
