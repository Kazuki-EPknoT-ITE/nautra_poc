      ...
      <table className="w-full min-w-[900px] text-sm">
        ...
        <tbody>
          {rows.map(({ status, subjectLabel }) => (
            <tr key={status.task.id} className="border-b border-[var(--ui-hairline)] last:border-b-0">
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {board.byGroup.map(({ group, rows }) => (
        <section key={group} aria-label={PROCEDURE_GROUP_LABEL[group]} className="ui-card p-4">
      ...
      <section aria-label="証書の期限・鮮度" className="ui-card p-4">
